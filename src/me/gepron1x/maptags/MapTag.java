...
public ItemStack toItemStack() {
	...
	lore.add("Мир: "+location.getWorld().toString());
	int x = (int) location.getX(); int y = (int) location.getY(); int z = (int) location.getZ();
	lore.add(x+" "+y+" "+z);
	lore.add("Владелец: " + Bukkit.getPlayer(owner).getDisplayName());
	meta.setLore(lore);
	return icon;
...
public String getLocation() {
	String result = location.getWorld().toString() + ","+location.getX()+","+location.getY()+","+location.getZ();
	return result;
}
}
